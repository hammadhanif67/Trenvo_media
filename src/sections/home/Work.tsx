          ...
          <ul className="mt-12 grid gap-6 md:grid-cols-3">
            {WORK.map((item) => (
              <li key={item.slug}>
                <Link
                  to={`/work/${item.slug}`}
                  className="group flex h-full flex-col border border-line-dark p-6 [transition:background-color_180ms,border-color_180ms] hover:border-blue-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500"
                >
                  {/*
          ...
          <>
            <ul className="mt-12 grid gap-6 md:grid-cols-3">
              {AWAITING.map((slot) => (
                <li
                  key={slot.id}
                  className="flex h-full flex-col border border-dashed border-line-dark p-6"
                >
